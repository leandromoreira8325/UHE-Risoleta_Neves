...
def gerar_relatorio_pdf(dados_serie, imagens_2026, output_dir, area_oficial_ha, mapa_path):
    ...
    title_style = ParagraphStyle('TitleStyle', parent=styles['Heading1'], fontSize=15, textColor=colors.HexColor('#1f385c'), spaceAfter=8, alignment=1)
    subtitle_style = ParagraphStyle('SubTitleStyle', parent=styles['Normal'], fontSize=10, textColor=colors.HexColor('#595959'), spaceAfter=12, alignment=1)
    h2_style = ParagraphStyle('H2Style', parent=styles['Heading2'], fontSize=11, textColor=colors.HexColor('#1f385c'), spaceBefore=10, spaceAfter=8)
    ...
    texto_metodologia = (
        ...
        "é realizada via índices e razões espectrais (NDVI/MNDWI), isolando a biomassa aquática estritamente sobre a poligonal oficial "
    ...
    t = Table(tabela_data, colWidths=[80, 105, 115, 80, 150])
    ...
    texto_conc = (
        ...
        f"de referência da UHE Risoleta Neves (1.450,00 ha). O indicador permanece dentro dos parâmetros de estabilidade operacional."
    )
    story.append(Paragraph(texto_conc, body_style))
    
    # Página 2: Prancha Cartográfica
    if mapa_path and os.path.exists(mapa_path):
        ...
        story.append(Spacer(1, 10))
        story.append(Image(mapa_path, width=480, height=320))
